        ...
        <div className={`${userSettings.darkMode ? 'dark' : ''}`}>
                ...
                <div className=" grid gap-5 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-5 ">
                    {lab.challenges.map((challenge, index) => (
                        <div onClick={() => userCtfs.includes(challenge.id) ? null : openModal(challenge)} title={`Clique para abrir ${challenge.title}`} key={index} className={`p-4 ${userCtfs.includes(challenge.id) ? '  ' : ' '} lab-bg  mb-4 hover:shadow-smmm shadow-inner hover:shadow-primaryy cursor-pointer  border border-white/20 relative rounded`}>
                            ...
                            <div className="flex mb-2 font-bold justify-end">
                                {
                                    userCtfs.includes(challenge.id)

                                        ?
                                        <FaCheck className=' text-xl my-auto me-2 ' />
                                        :
                            ...
                            {
                                /*
                                 <Popup trigger={<div className='flex gap-1   absolute right-6 top-6'>
                                    ...
                                    {
                                        userCtfs.includes(challenge.id) ?
                                            <div className='text-sm p-1 hacker relative'>
                            ...
                            <br />
                            <h4 className='sm:text-xl font-semibold mb-2'>{challenge.title}</h4>


                            {

                                /*
                         <div className="gap-5 mb-2 flex flex-start justify-start  ">
            ...
            {isModalOpen && selectedLabChallenge && (
                <HakyModalDefault className='text-white sm:w-[40rem] h-[27rem] sm:p-10 py-10 px-3' setShow={closeModal} show={true} bgDefault={true} shadowDeault={true}>
                    ...
                    <p>
                        <span className='flex gap-4'>Link: <a target='__blank' href={selectedLabChallenge.link} className="text-yellow-600 underline flex gap-2">{selectedLabChallenge.link} <FaLink className='my-auto  ms-2' /></a></span>
                    ...
                    </p>
                    {
                        msg === 'wrong' ?
                            ...
                            <>
                                <div className="bg-red-400/30 text-sm  mb-2 text-white font-bold px-4 py-3 flex gap-4">
                            ...
                            msg === 'right' ?
                                <>
                                    <div className="bg-green-400/30 rounded-lg text-center text-white font-bold px-4 py-2 mb-2 flex flex-col gap-2">
                    ...
                    <input
                        ...
                        className='border-2 sm:text-md text-xs text-black outline-none focus-within:border-primary w-full py-2 px-2 rounded-md'
                    ...
                    <div className="text-end mt-7">
                        {
                            loadingC ?
                                ...
                                <Button disabled={msg == 'right'} text='Submeter' color='primary' className='click sm:text-md text-xs text-black ms-auto' onClick={handleSubmitFlag} />
                        }
                    </div>
                </HakyModalDefault>
            )}
